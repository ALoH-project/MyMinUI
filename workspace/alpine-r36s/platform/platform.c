...
static int ALPINE_openInputByName(const char* want) {
	...
	for (int i=0; i<64; i++) {
		...
		if (fd < 0) continue;
		name[0] = '\0';
		if (ioctl(fd, EVIOCGNAME(sizeof(name)), name) >= 0 && strcmp(name, want)==0) return fd;
		close(fd);
	}
	return -1;
}
...
void PLAT_initInput(void) {
	...
	// Alpine R36S: combined "GO-Super Gamepad" (buttons/dpad/sticks/volume) plus
	// the separate "rk8xx_pwrkey" power key (KEY_POWER) for MinUI sleep/poweroff.
	{
		...
		if (gp >= 0) {
			...
			_START_RAW  = RAW_START_353;  // 315
			_MENU_RAW   = RAW_MENU_353;   // 316
			_L3_RAW     = RAW_L3_353;     // 317
			_R3_RAW     = RAW_R3_353;     // 318
			LOG_info("Detected Alpine R36S GO-Super Gamepad\n");fflush(stdout);
			return;
		}
	}
	if (is353v) {
...
//	vid.numpages = 1;
	...
	for (int i = 0; i < res->count_connectors; ++i) {
		...
		for (int c = 0; c < conn->count_modes; c++) {
			...
			// Alpine R36S: pick the w*h mode closest to FIXED_HZ (60) instead of
			// blindly taking the last advertised mode (@50 here).
			int _wh = (conn->modes[c].vdisplay == h && conn->modes[c].hdisplay == w);
			int _diff = conn->modes[c].vrefresh - hz; if (_diff < 0) _diff = -_diff;
			int _take = (_wh && _diff < best_hz_diff);
			if (!_take && !_wh && best_hz_diff == 1000000 && res->count_connectors == 1) _take = 1;
			if (_wh && _take) best_hz_diff = _diff;
			if (_take) {
...
void PLAT_getBatteryStatus(int* is_charging, int* charge) {
	...
	close(charge_fd);
	i = MIN(i,4100000);
	*charge = getInt("/sys/class/power_supply/rk817-battery/capacity");
	LOG_info("Charging: %d, Raw battery: %i -> %d%%\n", *is_charging, i, *charge);

	char status[16];
	getFile("/sys/class/net/wlan0/operstate", status,16);
	{ char *_ip = PLAT_getIPAddress(); online = (_ip && _ip[0] && strncmp(_ip,"127.",4)!=0); if(_ip) free(_ip); }
}
...
void PLAT_setCPUSpeed(int speed) {
	...
	char cmd[512];
	//sh -c "echo -n 1512000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_setspeed"
	sprintf(cmd,"sh -c \"echo -n userspace > %s \" ; sh -c \"echo %i > %s\"", GOVERNOR_PATH, freq, GOVERNOR_CPUSPEED_PATH);
	if (freq) {
		system(cmd);
...
char* PLAT_getModel(void) {
	 if (is353v) {
		if (is353p){
			return "RG353P Alpine";
		} else {
			return "RG353V Alpine";
		}
	}
	if (isrgb30) {
		return "RGB30 Alpine";
	...
	if (isr36splus) {
		return "R36S Plus Alpine";
	}
	if (isrg351p) {
		return "RG351P Alpine";
	}
	if (isv10) {
		return "V10 Alpine";
	}
...
char* PLAT_getIPAddress(void) {
    ...
    fp = popen("ip -4 -o addr show scope global | grep -oE 'inet [0-9.]+' | cut -d' ' -f2 | head -1", "r");
    if (fp == NULL) {
