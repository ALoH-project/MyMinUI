...
// Alpine R36S: find an input device by EVIOCGNAME (no /dev/input/by-path here,
// and the event number is not guaranteed).
...
int main (int argc, char *argv[]) {
	...
	if (is353v==1) {
		...
		//r36s -> Alpine: single combined "GO-Super Gamepad" at a non-fixed event node
		inputs[0] = ALPINE_openInputByName("GO-Super Gamepad"); // buttons+dpad+sticks+volume
		inputs[1] = -1;
		inputs[2] = -1;
		_MENU_RAW = CODE_MENU_353;    // 316 (BTN_MODE)
